...
<div>
    ...
    <div id="slidebar" ref="playerbar" :class="{tra:flog,closetra:!flog}">
      ...
      <!-- 播放头部 -->
			<div>
				<p class="_title" @click="li()">播放列表</p>
      ...
      <!-- 播放列表 -->
			<div class="_playBoxTemp " style="overflow-y:auto;" ref='playBoxTemp'>
				<div class="_playBox">
          <!-- 播放的专辑 -->
          <div v-for="(i,index) in data" :key="index" v-if="data!='[object Object]'">
      ...
      <div class="grid-music-container">
        ...
        <div class="u-cover paused">
          <!-- <a href="/catalogue/ARS38068" data-pjax=""> -->
            <!-- <img src=" https://cdn.naxos.com/SharedFiles/images/cds/others/ARS38068.gif" title="Abel:&nbsp;undefined:&nbsp;I. Allegro maestoso"> -->
          <!-- </a> -->
            <!-- loding加载 -->
            <!-- <img src="" alt=""  v-show="loading"> --> 
            <img v-if="data=='[object Object]' || data==''" :src="album_pic" alt="当我在这里">    
        ...
        <div class="m-music-play-wrap" @click="hidevolume()">
          <!-- 专辑的详情信息 -->
            ...
            <div class="m-now-info" v-if="data=='[object Object]' || data==''">
                <div class="eps">
        ...
        <!-- <audio :src="audio.location" @play="onPlay"  @timeupdate="onTimeupdate" @canplay="canPlaySong" @pause='onPause' @error="loadError" @ended="endPlayNext" id="audioPlay" ref="audio" @loadedmetadata='onloadedmetadata' :loop='chloop' controls hidden="true" @playing='onPlaying'></audio> -->
        <audio ref="audio" 
        @pause="onPause"
        @play="onPlay"
        :src="audio.location" controls="controls"></audio>
      </div>
        </div>
    </div>
</div>
...
import {worldaddress} from "../api/api"
...
export default {
  ...
  created(){
    ...
    if(this.data!='[object Object]' && this.data!=null){
      for(var i=0;i<this.data.length;i++){
        for(var j=i+1;j<this.data.length;j++){
          if(this.data[i].work.trackGroupId==this.data[j].work.trackGroupId){
            this.data[j].work=[]
          }
        }
      }
      for(var i=0;i<this.data.length;i++){
        for(var j=i+1;j<this.data.length;j++){
          if(this.data[i].catalogue.catalogueName==this.data[j].catalogue.catalogueName){
  ...
  mounted() {
    ...
    //设置中间框的高度
    // console.log(this.$refs.playBoxTemp)
    var h = document.documentElement.clientHeight || document.body.clientHeight;
    this.curHeight =h - 305; //减去页面上固定高度heigh
    ...
    //     this.catalogueplay=i.catalogue
    //   })
...
.m-play-controls01 a, .m-play-controls02 a {
    display: inline-block;
...
.m-play-controls01 .u-play-btn, .m-play-controls02 .u-play-btn {
    display: inline-block;
    width: 30px;
    height: 30px;
    margin-right: 15px;
...
.u-play-btn.modeListoff, .u-play-btn.mode-list {
